...
class Scene:
    def __init__(
        ...
        if self.detection_model is not None:
            logging.info("Initialized Scene with YOLO+SAM+CLIP detection stack")
        else:
    ...
    def get_frontier_observation_and_detect_target(
        ...
        if self.detection_model is not None:
            self.detection_model.set_classes([target_obj_class])
            results = self.detection_model.infer(
            ...
            # pure VLM fallback: rely on semantic sensor only
    ...
    def update_scene_graph(
        ...
        """3D-Mem detection-stack version: YOLO + SAM + CLIP + scene graph update.

        Falls back to storing the raw image when detection models are not loaded.
        """
        # Fallback: pure VLM path (no detection models loaded)
        if self.detection_model is None or self.clip_model is None:
            return self._update_scene_graph_vlm(
                image_rgb, depth, intrinsics, cam_pos, pts, pts_voxel,
                img_path, frame_idx, target_obj_mask,
            )
    ...
    def _update_scene_graph_vlm(
        ...
        """Pure VLM fallback: Store the image directly and pass to VLM without any preprocessing"""
        added_obj_ids = []
        ...
        target_obj_id = None  # We don't detect specific objects in pure VLM mode
    ...
    def make_detection_list_from_pcd_and_gobs(
        ...
        for mask_idx in range(len(gobs["mask"])):
            ...
            detected_object = {
                ...
                "image": None,
                # all frames this object has been seen in (parallel record, not used for snapshot logic)
    ...
    def update_snapshots(
        ...
        With detection stack (3D-Mem): hierarchical clustering of object ids.
        Without detection stack (pure VLM): add all new frames as snapshots,
        respecting previous VLM filtering decisions.
        """
        # Pure VLM path
        if self.detection_model is None or len(self.objects) == 0:
            for filename, frame in self.frames.items():
                if (
                    filename not in self.snapshots
                    and filename not in self.filtered_snapshots
                ):
                    self.snapshots[filename] = frame
            return

        # Detection-stack path (3D-Mem)
        if obj_ids is None:
            obj_ids = set()
    ...
    def periodic_cleanup_objects(self, frame_idx, pts):
        ...
        # Pure VLM path: no complex object cleanup needed
    ...
    def sanity_check(self, cfg):
        ...
        """
        if self.detection_model is None or len(self.objects) == 0:
            # Pure VLM approach basic checks
